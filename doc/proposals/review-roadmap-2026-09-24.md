...
- 前端真实输入 A/B（2026-09-25 23:33，hh 在有 Accessibility 授权的 Terminal 里按性能证据协议执行；A = v0.81.0 Release `d1ef71d65d911bb9`，B = e9dc3fb Release `2191d7a6fcfd32d8`，快照库 `bfcf282096b49a1f`，M3 Pro 36 GB，macOS 15.7.3，Xcode 26.1.1，每侧一次预热丢弃，3 对 A/A + 3 对 ABBA，原始数据 `logs/perf-ab-2026-09-25_23-33-18/`）：

  | 工作负载 | 指标 | A（v0.81.0） | B（e9dc3fb） | 判定 |
  | --- | --- | --- | --- | --- |
  | 搜索打字 "markdown" 8 键/秒 | Scopy CPU（打字 + 2 s 尾） | 1.78 ± 0.58 s | 1.75 ± 0.59 s | 噪声内（A/A 噪声 0.38） |
  | 同上 | List body 重跑次数 | 17.7 ± 0.5 | 12.2 ± 0.4 | −31%，满足协议 |
  | 同上 | 行初始化次数 `row.init` | 780 ± 21 | 450 ± 18 | −42%，满足协议 |
  | 滚轮滚动 12 s（约 2,850 px/s） | Scopy CPU | 3.16 ± 0.10 s | 3.22 ± 0.08 s | 噪声内 |
  | 同上 | 主线程 run-loop 忙时 | 2,353 ± 68 ms | 2,369 ± 54 ms | 噪声内 |
  | 同上 | 活动帧 p95 / max | 8.33 / 19.4 ms | 8.33 / 18.1 ms | 相同 / 噪声内 |
  | 同上 | 行初始化次数 | 1,106 ± 1 | 756 ± 1 | −32%，满足协议（B 每页 300 行，加载了 365 行对 165 行） |

  结论：两轮改动把每次按键和每页加载的结构性工作量明显减少（List body 重跑 −31%、行初始化 −32% 到 −42%），但端到端 CPU 与帧时间在噪声内没有变化，也没有回归；滚动帧 p95 两侧都已在显示节拍（8.33 ms）。搜索模式下 app 探针不记录 run-loop 忙时，该指标对打字无效。
