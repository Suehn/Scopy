...
|------|------|
| **当前版本** | v0.43.3 |
| **测试状态** | 单元测试通过（`make test-unit`: 53 passed, 1 skipped；`make test-perf`: 16 passed, 6 skipped；`make test-tsan`: 132 passed, 1 skipped；Strict Concurrency: `make test-strict` 160 passed, 7 skipped） |
| **构建状态** | Debug ✅ |
...
└── v0.43.3.md          ← Fix/Perf（短词全量校准 + 高速滚动降载，最新）
```
