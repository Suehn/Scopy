...
- **[v0.5-phase1.md](./v0.5-phase1.md)** - **最新** 测试流程自动化
  - test-flow.sh 自动化脚本 (编译 → 安装 → 启动 → 检查)
  - health-check.sh 6 项健康检查
  - Makefile 命令集成
  - 完整的测试验证和故障回滚机制

- **[v0.4.md](./v0.4.md)** - 用户可配置的设置窗口
  - 设置窗口 UI (Form + Picker + Toggle)
  - ⌘, 快捷键打开设置
  - UserDefaults 持久化
  - 完整的测试过程和验证

- **[v0.3.1.md](./v0.3.1.md)** - 大图片性能优化
  - 轻量级图片指纹算法
  - 主线程性能优化 (50×+)
  - 去重功能验证
  - 用户无感知体验

- **[v0.3.md](./v0.3.md)** - 前后端完整联调
  - 后端与前端完整集成
  - 全局快捷键 (⇧⌘C)
  - 搜索功能端到端验证
  - 核心功能全部可用

- **[v0.2.md](./v0.2.md)** - 后端完整实现
  - ClipboardMonitor 系统剪贴板监控
  - StorageService SQLite + FTS5 存储
  - SearchService 多模式搜索
  - 完整测试套件和性能基准

- **[v0.1.md](./v0.1.md)** - 前端初始实现
  - 实现路线和设计原则
  - UI组件和Mock后端
  - 使用教程和代码示例
