# v0.5 Test Framework - Quick Walkthrough

> 快速上手指南 - 帮助新启动的对话快速理解当前测试框架

---

## 🚀 5 分钟快速上手

### 1. 运行所有测试
```bash
xcodegen generate
xcodebuild test -scheme Scopy -destination 'platform=macOS' -only-testing:ScopyTests
```

### 2. 查看测试结果
```
Total: 69 tests
├── Unit Tests: 48/48 ✅
└── UI Tests: 21/21 ✅
```

### 3. 关键目录结构
```
ScopyTests/
├── AppStateTests.swift          # 31 个应用状态测试
├── PerformanceTests.swift       # 13 个性能测试
├── SearchServiceTests.swift
├── StorageServiceTests.swift
├── IntegrationTests.swift
└── Helpers/                     # 测试基础设施
    ├── TestDataFactory.swift     # 数据生成工厂
    ├── MockServices.swift        # Mock 服务
    ├── PerformanceHelpers.swift  # 性能测量工具
    └── XCTestExtensions.swift    # 异步测试扩展

ScopyUITests/                    # UI 测试 (独立 bundle)
├── MainWindowUITests.swift
├── HistoryListUITests.swift
├── KeyboardNavigationUITests.swift
├── SettingsUITests.swift
└── ContextMenuUITests.swift
```

---

## 📖 关键文件说明

### TestDataFactory.swift - 数据生成

**快速创建测试数据**:
```swift
// 创建 100 个测试项目
let items = TestDataFactory.makeItems(count: 100)

// 创建混合数据 (多个应用)
let mixed = TestDataFactory.makeMixedItems(count: 50)

// 创建特定搜索请求
let req = TestDataFactory.makeSearchRequest("query", mode: .fuzzy)
```

### MockServices.swift - Mock 实现

**配置 Mock 服务**:
```swift
let mock = ReusableMockClipboardService()
mock.setItemCount(100)  // 快速设置数据
mock.searchDelay = 0.1  // 模拟延迟
mock.resetCallCounts()  // 重置调用计数

// 验证调用
XCTAssertEqual(mock.searchCallCount, 1)
XCTAssertEqual(mock.lastSearchQuery, "test")
```

### PerformanceHelpers.swift - 性能测量

**快速测量性能**:
```swift
let stats = PerformanceHelpers.calculateStats(samples)
print(stats.report(title: "搜索性能"))

// 验证 SLO
SLOVerification.verifySearchLatency(
    stats: stats,
    target: .small  // ≤5k items, P95 ≤ 50ms
)
```

### XCTestExtensions.swift - 异步工具

**异步测试**:
```swift
// 等待条件满足
await waitForCondition(timeout: 5) { condition }

// 断言在时间内完成
try await assertCompletesWithin(1.0) {
    try await operation()
}
```

---

## 🎯 常见测试场景

### 场景 1: 测试搜索防抖
```swift
@MainActor
func testSearchDebounce() async throws {
    let mock = TestMockClipboardService()
    mock.setItemCount(100)

    // 模拟快速输入
    appState.searchQuery = "h"
    appState.search()
    appState.searchQuery = "he"
    appState.search()

    // 应该防抖（在 150ms 后执行）
    try await Task.sleep(nanoseconds: 100_000_000)
    XCTAssertEqual(mock.searchCallCount, 0)

    try await Task.sleep(nanoseconds: 100_000_000)
    XCTAssertEqual(mock.searchCallCount, 1)
}
```

### 场景 2: 测试性能达成 SLO
```swift
func testSearchPerformance() async throws {
    // 创建 5000 个项目
    for i in 0..<5000 {
        _ = try storage.upsertItem(makeContent("Item \(i)"))
    }

    var times: [Double] = []
    for query in ["Hello", "test", "random"] {
        let start = CFAbsoluteTimeGetCurrent()
        _ = try await search.search(query: query, ...)
        times.append((CFAbsoluteTimeGetCurrent() - start) * 1000)
    }

    // 计算 P95
    times.sort()
    let p95 = times[Int(Double(times.count) * 0.95)]

    // v0.md 目标: ≤5k items, P95 ≤ 50ms
    XCTAssertLessThan(p95, 50)
}
```

### 场景 3: UI 测试
```swift
func testMainWindowLayout() throws {
    let app = XCUIApplication()
    app.launchArguments = ["--uitesting"]
    app.launch()

    // 验证搜索字段存在
    let searchField = app.searchFields.firstMatch
    XCTAssertTrue(searchField.waitForExistence(timeout: 5))

    // 验证列表存在
    let list = app.scrollViews.firstMatch
    XCTAssertTrue(list.exists)
}
```

---

## 🔧 修改和扩展

### 添加新的单元测试

1. **在 AppStateTests 中添加**:
```swift
func testMyNewFeature() async {
    mockService.setItemCount(50)
    await appState.load()

    // 测试逻辑
    // ...

    XCTAssertEqual(...)
}
```

2. **使用 Mock 验证调用**:
```swift
XCTAssertEqual(mockService.copyCallCount, 1)
XCTAssertEqual(mockService.lastCopiedItemID, itemId)
```

### 添加新的 UI 测试

1. **创建新文件**: `ScopyUITests/MyFeatureUITests.swift`
2. **使用标准模板**:
```swift
import XCTest

final class MyFeatureUITests: XCTestCase {
    var app: XCUIApplication!

    override func setUpWithError() throws {
        continueAfterFailure = false
        app = XCUIApplication()
        app.launchArguments = ["--uitesting"]
        app.launch()
    }

    func testMyFeature() throws {
        // 测试实现
    }
}
```

### 添加新的性能测试

使用 `PerformanceHelpers`:
```swift
func testMyOperationPerformance() async throws {
    let stats = await runPerformanceTest(iterations: 20) {
        try await operation()
    }

    // 验证性能
    XCTAssertLessThan(stats.p95, 50)
}
```

---

## 📊 v0.md SLO 对齐

### 当前测试覆盖的 SLO

| 指标 | 目标 | 验证方式 |
|------|------|--------|
| 首屏加载 | P95 < 100ms | `testFirstScreenLoadPerformance` |
| 搜索 (≤5k) | P95 < 50ms | `testSearchPerformance5kItems` |
| 搜索 (10k) | P95 < 150ms | `testSearchPerformance10kItems` |
| 搜索防抖 | 150-200ms | `testSearchDebounceEffect` |
| 内存稳定性 | < 50MB (500 ops) | `testMemoryStability` |

### 运行 SLO 验证
```swift
let stats = PerformanceHelpers.calculateStats(times)
SLOVerification.verifySearchLatency(stats: stats, target: .small)
```

---

## 🐛 常见问题与调试

### 问题 1: "testEventStream" 测试较弱
**原因**: 异步事件流在测试环境中不稳定

**解决**: 直接测试事件处理逻辑而非 stream
```swift
// ✅ 推荐
appState.items.insert(newItem, at: 0)
XCTAssertEqual(appState.items.first?.id, newItem.id)

// ❌ 避免
try await taskForStream.value  // 不稳定
```

### 问题 2: UI 测试找不到元素
**原因**: 元素加载延迟

**解决**: 使用 waitForExistence
```swift
let element = app.scrollViews.firstMatch
XCTAssertTrue(element.waitForExistence(timeout: 5))
```

### 问题 3: 性能测试波动大
**原因**: 系统负载、缓存影响

**解决**: 使用 P95 而非平均值，多次采样
```swift
let samples = try await PerformanceHelpers.collectTimeSamplesAsync(
    iterations: 20,
    warmupIterations: 2
) { try await operation() }
```

---

## 📈 下一步开发

### v0.6 前端 UI 实现
- 扩展 UI 测试覆盖复杂交互
- 添加拖拽、多窗口等高级功能测试
- 集成性能测试到 CI

### v0.7 集成测试
- 完整的数据流测试 (Monitor → Storage → Search → UI)
- 端到端场景测试
- 性能回归检测

### v0.8 生产就绪
- 100% 覆盖率目标
- 自动化性能基准线
- 完整的 CI/CD 集成

---

## 📚 重要链接

- **完整文档**: `doc/implemented-doc/v0.5.md`
- **设计规范**: `dev-doc/v0.md`
- **协议定义**: `Scopy/Protocols/ClipboardServiceProtocol.swift`

---

## ⏱️ 关键时间线

```
2025-11-27
├── Phase 1: SearchService 修复 ✅
├── Phase 2: AppState 测试 ✅
├── Phase 3: UI 测试框架 ✅
├── Phase 4: 性能测试扩展 ✅
└── Phase 5: 测试基础设施 ✅

下一步: v0.6 前端 UI 实现
```

---

**版本**: v0.5 Walkthrough
**最后更新**: 2025-11-27
**状态**: ✅ 完成
