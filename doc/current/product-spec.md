...
- Recognize only validated `scopy-rich` v2 envelopes for structured web results, search-image groups, news, weather, finance, and currency data cards. The sole public-Markdown presentation adapter may group two or more consecutive image-only paragraphs using only their visible `src`/`alt`/`title` fields; it must not infer card type, source, citation, date, article data, or other private metadata. Rich data surfaces use frozen source data and bundled allowlisted assets; invalid or unsupported envelopes remain visible code fences, and ordinary copied prose must never be guessed into private card metadata.
