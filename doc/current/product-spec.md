...
- The reusable Markdown `WKWebView` has at most one current visible owner. An unowned hover prewarm may preload the next document without publishing readiness or changing visibility, and a hidden document never fails on its paint deadline; stale teardown cannot detach a newer owner, identical in-flight HTML cannot restart navigation, and scroll configuration must tolerate WebKit creating its internal scroll view after the initial update.
