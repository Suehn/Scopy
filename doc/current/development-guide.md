...
12. Markdown table cells use the captured TableContainer component contract, not the older native Markdown table padding contract: base cell horizontal padding is 8px, first-cell start padding is 0, last-cell end padding is 0, header-row block padding is 8px, body-row block padding is 10px, and the last row removes only the bottom border. Keep the inner table wrapper with `width: fit-content` and `min-width: 100%`. Column sizes are `xs/sm/md/lg/xl` from the WACZ JS thresholds `>160/>100/>40/>4/else`. TableContainer column widths are based on the component baseline `640px` or `768px` at `min-width: 1024px`; do not bind them to Scopy's current message/content width, because browser zoom should be able to make a table overflow horizontally instead of forcing more cell wrapping. Do not add export-only last-column width or padding rules to solve bitmap clipping. Infer per-column sizes in the shared renderer for all tables and keep export as a scale transform, not a separate table stylesheet.
