...
19. Search index disk caches use `SearchIndexBinaryCodec` (`*.shortindex.v3.bin`, `*.fullindex.v6.bin`): the header carries the mutation sequence so a stale cache is rejected before checksum and decode, and decoding a 15 MB short index takes about 55 ms instead of 2.9 s. Do not reintroduce property-list encoding for postings or item tables.
