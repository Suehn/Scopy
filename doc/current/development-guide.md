...
8. The ChatGPT-aligned Markdown theme uses the captured `markdown markdown-new-styling` rules for heading, paragraph, list, and blockquote rhythm; do not fall back to the older AssistantMessage `qN-_1G_MarkdownContent` heading scale unless a new capture proves the DOM has changed again.
