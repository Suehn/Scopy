...
- Splits ChatGPT Markdown layout scale from output size: Scopy now defaults to the `100%` desktop profile (`48rem`/`768px` content column inside an `816px` render surface), adds a `125%` profile (`40rem`/`640px` content column with scaled font metrics), and keeps PNG target pixels plus hover-preview shell width stable across profiles.
...
- Re-anchors the ChatGPT width model around explicit layout profiles instead of a single hard-coded column: `100%` is the default desktop `48rem` path, while `125%` remains available for the narrower measured `40rem` path.
