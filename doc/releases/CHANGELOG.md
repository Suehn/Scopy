...
- 内置 pngquant 改用 fork `perf2` 分支构建（CLI `624df85`，引擎 `b664d56`），并合入 9 月 5–6 日从未发布过的 phase 3/4 引擎优化；仍为通用二进制、ad-hoc 签名，来源与重建方式见 `Scopy/Resources/ThirdParty/pngquant/PROVENANCE.md`。设置、默认值和 `PngquantService` 不变。
- 抖动跳过没有误差的平坦区域：恰为调色板颜色且没有入射误差的像素、同色游程和整行纯色直接写出，结果逐字节不变。
- speed 1 的抖动改为多核分块（每块至少 256 行，先扩散 16 行），得到等价的抖动图样：参考导出页 0.14% 像素排列不同，MSE 与模糊后 MSE 持平。
- speed 2–10 解码后像素不变；高于 2,816 行的长图分块更细，接缝落在别的行（参考导出页 0.05% 像素，MSE 持平）。
- PNG 写出先比较左邻字节和上方行，长图的平坦分块用较浅的搜索；8 位图直接在扫描线缓冲区上压缩。文件大小在 −3.3% 到 +0.9% 之间。

### Performance

- 参考导出页（2160×29511）pngquant 整体耗时：默认导出设置 207 → 99 ms（2.1×），speed 1 / 16 色 861 → 159 ms（5.4×）；`chatgpt_rich_surfaces` 页 237 → 159 ms（1.5×）；4 Mpx 截图 80 → 72 ms，主要受 PNG 解码限制。

## [v0.80.7] - 2026-09-11

### Fixed

- 去掉文字、图片和文件预览顶部预留的工具栏横条，改为边角悬浮控件，让内容与窗口保持一体。
- 恢复 v0.80.6 之前的 Markdown 排版与缩放：相同缩放值保留原来的换行，窄窗口只适配显示，宽窗口居中且不额外放大。
- 保留多个独立固定预览窗口、窗口缩放和状态；PNG 导出保持原样。
...
- 支持多个固定预览窗口并存，固定后自动隐藏历史面板；重新打开列表仍可继续悬浮预览其他条目。
- 悬浮预览支持拖动角落调整尺寸；固定窗口保留独立的缩放、滚动和尺寸，删除一个条目只关闭对应窗口。
- 顶部统一为淡化工具栏，悬停时清晰显示；移除重复标题栏和原始 Markdown 标题，控件不再遮住正文。
- 窗口调整宽度时正文重新排版并居中，宽代码与表格局部滚动；PNG 保留固定输出布局。恢复的窗口位置适配当前屏幕。

## [v0.80.5] - 2026-09-11

### Fixed

- 网站图标改为通用来源服务：普通链接、引用、结果卡片按来源获取并缓存 favicon，不再依赖少量内置域名。
- 仅读取网站主页和图标，支持站点声明、相对地址、ICO 回退、去重、离线缓存和失败回退；新增独立的网站图标设置。
- 预览与 PNG 共用同一图标服务；修复慢图标导致 WebKit 字体就绪超时，保留图标原色与行内排版。

## [v0.80.4] - 2026-09-05

### Fixed

- 网页链接、引用与搜索结果补齐来源图标，加入大象银行和汇丰官方 favicon；冻结图标有总量限制。
- 文件、图片、音视频、Office、代码类型与 22 种应用标注采用 Codex 原始 SVG，保留形状、原色、渐变及精细间距。
- 修复 `plugin://` 品牌识别遗漏、SVG 评分与地图颜色丢失，以及 PNG 调色板压缩导致的图标偏色。
- 预览与导出共用同一渲染链；应用标注保持不可导航。汇率选择器上下箭头继续使用原有图形。

## [v0.80.3] - 2026-09-05

### Fixed

- 修复 GPT 网页双格式复制时，中文 Markdown 与 HTML 被误判为不同内容，导致表格压平、加粗丢失的问题；保留原始 Markdown 和 HTML。
- 增加中文表格、加粗、金额与公式的捕获回归覆盖。普通与严格并发配置各通过 833 项 XCTest（4 项条件跳过），实机 PNG 验证通过。已丢失格式的旧记录需重新复制。
...
- 中文与标点相邻的加粗、包裹公式的加粗正常渲染，例如 `这是**重点。**请注意`。
- 支持单美元公式 `$x_i+y_j$` 及中文紧邻的 `公式$x$之后`，同时保持 `$5 and $10`、`US$5 to US$10`、金额区间、代码和链接地址为字面量。公式须在同一行且定界符内侧无空白；明确配对的 `$5$` 视为数学，字面美元对可用转义或代码表示。
- KaTeX HTML 的 TeX annotation 即使只有普通变量，也能被剪贴板解析器正确识别。

### Changed

- 预览和 PNG 继续共享同一套渲染器，同步更新资源校验和并使旧渲染缓存失效。
- 剪贴板测试改用真实生产捕获链，删除重复解析器、无消费者的设置订阅、闲置辅助代码和无效断言测试，并精简重复文档及 skill 指令。
- Node 147 项通过；普通与严格并发构建各通过 830 项 XCTest（各 4 项条件跳过）；真实应用 PNG 已检查。测试调度环境限制和复现图见 [发布记录](./history/v0.80.2.md)。
...
- Re-anchors the ChatGPT zoom model around explicit layout profiles instead of canvas scaling: `100%` and `125%` share the same output surface, while `125%` reflows in a narrower CSS viewport before being zoomed back to the fixed preview/export width.
...
- Focused hover-preview layout-scale strict regression: `HistoryHoverPreviewPipelineTests` passed with `SWIFT_STRICT_CONCURRENCY=complete`; executed 13 tests, 0 failures, and no `HistoryHoverPreviewPipeline.swift` warning/error remained (2026-05-30).
