...
- `doc/specs/v0-supplement.md`  
  - 新增补充设计（稳定性/性能）存档。
- 版本文档与索引更新：`doc/implementation/README.md`、`doc/implementation/CHANGELOG.md`、`DEPLOYMENT.md`。

## 🎯 关键指标（本次实测）
- 单测：`xcodebuild test -scheme Scopy -destination 'platform=macOS' -only-testing:ScopyTests` **161/161 通过，1 跳过（需 RUN_PERF_TESTS）**
- 性能（节选，详见 `doc/profiles/v0.16-profile.md`）：
...
## 🔮 遗留与后续
- 补充性能数据：运行性能测试集，更新 `DEPLOYMENT.md` 与 `doc/profiles/` 对比文档。  
