...
> 目标：对照 `doc/specs/v0.md` 的性能目标与当前实现（含 v0.27 全量模糊搜索 + P0 性能修复），定位真实热点与成因，给出可从原理上优化的点。  
...
- **v0.26–v0.29 已落地主要 P0–P2**：
  - v0.26：热路径清理节流、缩略图冷加载后台化、短词全量 fuzzy 去噪。
  - v0.27：搜索/分页版本一致性，避免旧结果混入（“后面出现不相关 item”竞态）。
  - v0.28：全量 fuzzy 重载提速（postings 有序交集 + top‑K 堆 + 首屏 FTS 预筛）与图片管线全链路后台化。
  - v0.29：**渐进搜索全量校准**（fuzzy/fuzzyPlus 首屏预筛 + 后台 refine）与 **预筛首屏/分页一致性补强**；全量索引内存缩减、外部写入后台化、NSCache/ObservationIgnored 等 P1/P2 收敛。
- **当前剩余风险主要是 P2 结构性优化**（如 postings 空洞后台重建、SearchService actor 化、重复字符候选压缩），对体验不再形成 P0/P1 级阻塞。
...
- **P0（体验/准确性）**：已在 v0.28–v0.29 完成（重载 fuzzy 提速、图片后台化、渐进校准 + 预筛分页一致性）。
- **P1（规模化与主线程重活）**
  1. SearchService actor 化统一并发语义（当前 queue+锁已稳定，可按需演进）。
  2. AppState 进一步拆分 Search/List/Stats（如后续仍观测到重绘半径过大）。
- **P2（结构/细节）**
  1. postings “空洞比例”过高时后台重建索引（长期运行场景）。
  2. 重复字符候选压缩（字符计数等）。
  3. WAL/ vacuum idle 调度进一步智能化。
