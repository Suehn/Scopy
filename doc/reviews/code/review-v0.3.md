...
> 说明：本文用于指导后续“稳定性优先”的长期重构（含 Codex 执行）。`doc/reviews/review-v0.3-2.md` 为历史草案/补充材料，其中关键内容已合并到本文；后续以本文为准。

- 最后更新：2025-12-14
- 代码基线：`v0.43.10`
- 重构状态：Phase 0-7 已完成（至 `v0.43.10`）
- 关联文档：
  - 当前实现状态索引：`doc/implementation/README.md`
  - 近期变更：`doc/implementation/CHANGELOG.md`
  - 规格参考：`doc/specs/v0.md`
...
- 每个 Phase 完成视为一次“开发完成”：必须更新 `doc/implementation/releases/vX.X(.X).md`、`doc/implementation/README.md`、`doc/implementation/CHANGELOG.md`（按 `CLAUDE.md`）。
...
- 已完成（2025-12-13，v0.41）：
  - （可选）将 Strict Concurrency 固化为 CI/Makefile 回归门槛：
    - Makefile 新增 `make test-strict`：
      ...
      - 输出写入 `logs/strict-concurrency-test.log` 便于审计
...
4. 对 `doc/implementation/*` 的同步更新（按 `CLAUDE.md`）

推荐给 Codex 的输入模板：

```
请按 doc/reviews/review-v0.3.md 的 Phase {N} 执行重构。
...
- 文件清单 + 关键代码点
- 运行命令与结果
- 下一阶段注意事项（写入 doc/reviews/review-v0.3.md 对应 Phase Notes）
```
