...
**基于版本**: v0.44.fix20（见 `doc/implementation/README.md`，最后更新 2025-12-16）  
**基于提交**: 7418acc  
**Review 范围**: 稳定性 / 性能（前后端）/ 规格实现准确性（对照 `doc/specs/v0.md`）/ 代码优雅性与可维护性  
...
1. 阅读：`doc/implementation/README.md` / `doc/implementation/CHANGELOG.md` / `doc/specs/v0.md`，明确当前版本与验收口径。
...
| P2-3 | P2 | 规格偏离 | FTS 排序未纳入匹配度（bm25/rank），与 v0.md “匹配度+最近使用”不一致 | `Scopy/Infrastructure/Search/SearchEngineImpl.swift:1165-1216`，`doc/specs/v0.md:158-163` |
| P2-4 | P2 | 规格偏离 / 存储策略 | 大文本未外部化：`.text` payload 为 `.none`，全文仍入库/FTS | `Scopy/Services/ClipboardMonitor.swift:584-593`，`Scopy/Services/StorageService.swift:244-271`，`doc/specs/v0.md:50-53` |
...
- `doc/specs/v0.md` 4.2 “短词/长词优化”写的是“短词可以先在内存缓存最近 N 条快速过滤”，语义上更像**prefilter**而不是“只搜最近 N 条作为最终结果”。当前 fuzzy 已按 prefilter 思路实现（短词先 cache，随后 refine full fuzzy），但 exact 没有同样机制。
...
- `doc/specs/v0.md` 4.3（约 158–163 行附近）写的是：默认“匹配度 + 最近使用时间组合排序”。
...
- `doc/specs/v0.md` 2.1：分级存储写到“图片 / 大文本 / 文件 ≥ X KB 外部文件 + DB 存 path/元数据”。
- 现实现：
