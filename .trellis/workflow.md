...
Before spawning the implement sub-agent, the main session MUST make the task active
...
Do not spawn `trellis-implement` until `current --source` prints the expected
task path in the main session. If it prints `Current task: (none)`, rerun
`task.py start <task-dir>` in the main session and verify again. Also include
the verified task path in the sub-agent prompt as `TASK_DIR=<task-dir>`.
Codex sub-agents run in isolated sessions, so they must load task context from
explicit `TASK_DIR` first and verify that `<task-dir>/prd.md` exists; their own
`current --source` is only a fallback/diagnostic and may legitimately be none.
...
Before spawning the check sub-agent, the main session MUST make the task active
for the current Codex session:
...
Do not spawn `trellis-check` until `current --source` prints the expected task
path in the main session. If it prints `Current task: (none)`, rerun
`task.py start <task-dir>` in the main session and verify again. Also include
the verified task path in the sub-agent prompt as `TASK_DIR=<task-dir>`.
Codex sub-agents run in isolated sessions, so they must load task context from
explicit `TASK_DIR` first and verify that `<task-dir>/prd.md` exists; their own
