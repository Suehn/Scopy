...
- Tagged packaging checksum: `.build/Scopy-<version>.dmg.sha256`, generated by the same `scripts/build-release.sh` invocation as the DMG.
- `.github/workflows/release.yml` must call `scripts/build-release.sh` with `SCOPY_XCODE_DERIVED_DATA` under `${{ runner.temp }}` and upload the two `.build/Scopy-*` artifacts. CI must not duplicate or guess the Xcode product path.
...
| CI Xcode build succeeds but packaging references `.build/Release/Scopy.app` | Reject in the release-policy regression; Xcode products live only in DerivedData |
...
- Good: local and GitHub tagged packaging both execute `scripts/build-release.sh`; the workflow only selects a safe runner DerivedData root and uploads the script's DMG/SHA outputs.
...
- Bad: a workflow runs its own `xcodebuild` and later copies `.build/Release/Scopy.app`; that path is incompatible with the required DerivedData isolation even when compilation succeeds.
...
7. Run `make test-release-policy`; assert the real workflow calls the tagged packager, contains no `.build/Release/Scopy.app`, and the packager emits both DMG and SHA sidecar.
...
```yaml
# Wrong: duplicates the build and guesses a removed Xcode output path.
- run: xcodebuild -scheme Scopy -configuration Release build
- run: cp -r .build/Release/Scopy.app dmg/

# Correct: one tagged packager owns build, app validation, DMG, and SHA paths.
- env:
    SCOPY_XCODE_DERIVED_DATA: ${{ runner.temp }}/Scopy-Release
