...
_FALLBACK_BREADCRUMBS = {
    "no_task": (
        "No active task.\n"
        "Codex sub-agent override: if the current prompt contains "
        "`TASK_DIR=<task-dir>`, verify `<task-dir>/prd.md`, treat that "
        "directory as the authoritative task source, and proceed. In that "
        "case this no-task breadcrumb and `task.py current --source` returning "
        "`Current task: (none)` are diagnostic only; do not create a new task "
        "or ask the parent to choose or restate a task.\n"
    ...
    "in_progress": (
        ...
        "For TASK_DIR-critical Trellis work these exact types are required. "
        "If GPT-5.5/high/another model override is needed, apply it to the "
        "exact Trellis agent type instead of switching to a generic/default/"
        "explorer agent. Generic/default/explorer/generalPurpose sub-agents "
        "do not receive Trellis role preludes or `implement.jsonl` / "
        "`check.jsonl` loading contracts; use them "
        "only for non-authoritative scouting or non-Trellis work. For Trellis "
        "TASK_DIR work they are forbidden unless the main session explicitly "
        "accepts a known-fallible diagnostic-only path; then the parent prompt "
        "MUST include a prelude telling it to verify `<task-dir>/prd.md` from "
        "explicit `TASK_DIR=<task-dir>`, treat that TASK_DIR as authoritative, "
        "and treat local no-task/current-none state as diagnostic only.\n"
        "User override (per-turn escape hatch): if the user's CURRENT message "
