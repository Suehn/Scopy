...
final class ClipboardMonitorTests: XCTestCase {
    ...
    func testMonitorEmitsMarkerBearingTextAndContinuesMonitoring() async {
        let markerTypes = [
        ...
        let markerTexts = markerTypes.map { "marker-bearing-\($0)-\(UUID())" }
        let sentinelText = "post-marker-sentinel-\(UUID())"
        let expectedTexts = markerTexts + [sentinelText]
        let expectations = expectedTexts.map {
            XCTestExpectation(description: "Monitor emitted \($0)")
        }
        var receivedTypes: [String: ClipboardItemType] = [:]
        ...
        let task = Task {
            for await content in monitor.contentStream {
                guard let index = expectedTexts.firstIndex(of: content.plainText) else { continue }
                receivedTypes[content.plainText] = content.type
                expectations[index].fulfill()
            }
        }
        defer { task.cancel() }

        for index in markerTypes.indices {
            pasteboard.clearContents()
            let item = NSPasteboardItem()
            XCTAssertTrue(item.setString(markerTexts[index], forType: .string))
            XCTAssertTrue(item.setData(Data([0x01]), forType: NSPasteboard.PasteboardType(markerTypes[index])))
            XCTAssertTrue(pasteboard.writeObjects([item]))
            await fulfillment(of: [expectations[index]], timeout: 2.0)
        }

        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.setString(sentinelText, forType: .string))
        await fulfillment(of: [expectations[markerTypes.count]], timeout: 2.0)

        XCTAssertEqual(receivedTypes.count, expectedTexts.count)
        XCTAssertTrue(expectedTexts.allSatisfy { receivedTypes[$0] == .text })
    }
