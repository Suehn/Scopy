...
final class ClipboardMonitorTests: XCTestCase {
    ...
    func testMonitorNeverEmitsProtectedPasteboardItems() async {
        let protectedTypes = [
        ...
        let protectedTexts = protectedTypes.map { "protected-\($0)-\(UUID())" }
        let companionTexts = protectedTypes.map { "unprotected-companion-\($0)-\(UUID())" }
        let sentinelTexts = protectedTypes.map { "sentinel-\($0)-\(UUID())" }
        ...
        let sentinelExpectations = sentinelTexts.map {
            XCTestExpectation(description: "Monitor continued after protected type \($0)")
        }
        let task = Task {
            for await content in monitor.contentStream {
                receivedTexts.append(content.plainText)
                if let index = sentinelTexts.firstIndex(of: content.plainText) {
                    sentinelExpectations[index].fulfill()
                }
            }
        }
        defer { task.cancel() }

        for index in protectedTypes.indices {
            pasteboard.clearContents()
            let item = NSPasteboardItem()
            XCTAssertTrue(item.setString(protectedTexts[index], forType: .string))
            XCTAssertTrue(item.setData(Data([0x01]), forType: NSPasteboard.PasteboardType(protectedTypes[index])))
            let companion = NSPasteboardItem()
            XCTAssertTrue(companion.setString(companionTexts[index], forType: .string))
            XCTAssertTrue(pasteboard.writeObjects([item, companion]))

            try? await Task.sleep(nanoseconds: 300_000_000)

            pasteboard.clearContents()
            XCTAssertTrue(pasteboard.setString(sentinelTexts[index], forType: .string))
            await fulfillment(of: [sentinelExpectations[index]], timeout: 2.0)
        }

        XCTAssertTrue(Set(receivedTexts).isDisjoint(with: protectedTexts))
        XCTAssertTrue(Set(receivedTexts).isDisjoint(with: companionTexts))
    }
